...
def test_aws_deploy_passes_subnet_id_to_server(
    client: TestClient,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        provider,
        "create_network",
        lambda name, cidr=None: {"id": "vpc-1", "name": name, "status": "available"},
    )
    monkeypatch.setattr(
        provider,
        "create_subnet",
    ...
    assert response.status_code == 200
    assert server_calls == [
        {"name": "client-a", "network_id": "subnet-1"},
        {"name": "client-b", "network_id": "subnet-1"},
    ]
